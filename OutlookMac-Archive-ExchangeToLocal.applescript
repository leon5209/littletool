...
tell application "Microsoft Outlook"
	...
	set localMailArchiveRootFolderName to "RTK Archive Mail" -- name of the root archive mail folder to create under 'On My Computer'. If an existing archive mail folder is found it will use it, otherwise it will create the folder for you
...
on archiveMail(mailFolder, currentArchiveFolder, earliestDate)
	tell application "Microsoft Outlook"
		with timeout of (my minutesBeforeTimeOut) * 60 seconds
			...
			log "Leon there are " & (count of exchangeMessages) & " messages in folder " & (name of mailFolder as text)
				
				set theMessage to item theIncrementValue of exchangeMessages
				log "subject: " & (subject of theMessage as text)

				-- here it can not reach the very first mail in the mailFolder
				if time sent of theMessage is less than earliestDate then
				    ...
				    log "Mail is not finish due to it did no begin with the first mail"
